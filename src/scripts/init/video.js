...
function lockForIntro(video) {
   ...
   // Плавно перемотує на справжній останній кадр — ховаємо відео, чекаємо
   // на "seeked" (кадр реально готовий, не миттєво), і тільки тоді
   // проявляємо назад тим самим fade, що й на старті програвання. Без
   // цього прямий video.currentTime = video.duration видно як різкий
   // "стрибок" кадру.
   const seekToEndSmoothly = () => {
      if (!video.duration) return;
      if (Math.abs(video.currentTime - video.duration) < 0.05) return;
      ...
      setTimeout(reveal, 400); // safety net — раптом "seeked" не стрельне
   };

   // Вкладку/додаток згортають посеред відтворення — на поверненні відео
   ...
   // відео ненадійно. Перемотуємо на останній кадр ОДРАЗУ як вкладку
   // ховають (поки вона не видима — перехід непомітний), а не чекаємо
   // повернення.
   //
   // На поверненні — підстраховка: довге реальне перебування у фоні може
   // змусити мобільний браузер скинути декодований буфер на кадр з початку
   // джерела вже ПІСЛЯ нашої перемотки вище. Якщо це сталось — виправляємо
   // плавно (fade), а не жорстким стрибком.
